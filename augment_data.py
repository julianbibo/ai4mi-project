...
class MedicalImageAugmentor:
    ...
    def zoom(self, img, zoom_factor):
        ...
        if zoom_factor > 1:
            ...
            new_img.paste(
                img_resized, ((width - new_width) // 2, (height - new_height) // 2)
            )
            return new_img

    def translate(self, img, x_shift, y_shift):
        return img.transform(
            img.size, Image.AFFINE, (1, 0, x_shift, 0, 1, y_shift), fillcolor=0
        )
            ...
            for data_type in ["img", "gt"]:
                ...
                for img_file in tqdm(image_files, desc=f"{split}/{data_type}"):
                    ...
                    if not should_augment:
                        continue
                    if data_type == "img":
                        geom_augs = random.sample(
                            geometric_augs,
                            min(self.augmentations_per_image, len(geometric_augs)),
                        )
                        ...
                        if len(total_augs) < self.augmentations_per_image:
                            ...
                            total_augs.extend(
                                random.sample(
                                    intensity_augs, min(remaining, len(intensity_augs))
                                )
                            )
                    ...
                    for i, aug_type in enumerate(selected_augs):
                        aug_img = self.augment_single_image(
                            img, aug_type, is_mask=is_mask
                        )
                        aug_filename = f"{original_name}_aug_{i + 1}_{aug_type}.png"
                        aug_img.save(output_path / aug_filename)
                if should_augment:
                    print(
                        f"Completed {split}/{data_type}: {len(image_files) * (self.augmentations_per_image + 1)} total images"
                    )
                else:
                    print(
                        f"Completed {split}/{data_type}: {len(image_files)} original images (no augmentation)"
                    )
